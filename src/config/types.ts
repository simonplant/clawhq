...
/** Active hours range — inverted from quiet_hours for cron scheduling. */
export interface ActiveHours {
  /** Hour when active period starts (0-23). */
  readonly start: number;
  /** Hour when active period ends (0-23). */
  readonly end: number;
 ...
 *
 * Written to `cron/jobs.json` inside a `{ version: 1, jobs: [...] }` envelope.
 * The Gateway hot-reloads this file.
 *
 * Format verified from working OpenClaw deployments:
 *   { id, name, enabled, schedule, delivery, payload, sessionTarget?, state? }
 */
export interface CronJobDefinition {
  ...
  readonly delivery: { readonly mode: string };
  readonly payload: {
    readonly kind: "agentTurn";
    readonly message: string;
    readonly model?: string;
  };
  readonly sessionTarget?: "isolated" | "main";
  /** Model fallback chain (ClawHQ extension — stripped before writing jobs.json). */
  readonly fallbacks?: readonly string[];
  /** Active hours for waking-qualified jobs (ClawHQ extension — stripped before writing jobs.json). */
  readonly activeHours?: ActiveHours;
  /** Runtime state — preserved across apply, not generated by compiler. */

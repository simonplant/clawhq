...
export function compile(
  ...
  // Reuse an existing OPENCLAW_GATEWAY_TOKEN if the deployment already has one
  // set to a real (non-placeholder) value — rotating the token every compile
  // breaks every open session on every apply. Fresh init (existingEnv empty or
  // placeholder) still generates a random token.
...
function renderEnv(
  ...
  // literal "CHANGE_ME", and the undefined case as "no prior token" — all
  // three should trigger generation. Anything else survives across compiles
  // so apply is idempotent and doesn't orphan open sessions.
  const token =
    existingToken && existingToken.length > 0 && existingToken !== "CHANGE_ME"
      ? existingToken

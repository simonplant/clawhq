...
export function compile(
  ...
  const files: CompiledFile[] = [
    ...
    { relativePath: "engine/.env", content: renderEnv(gatewayPort), mode: 0o600 },
    ...
    // Ops
    { relativePath: "ops/firewall/allowlist.yaml", content: renderAllowlist(profile) },
...
function renderEnv(port: number): string {
...
function renderAllowlist(profile: MissionProfile): string {
  const lines = [
  ...
  for (const domain of profile.egress_domains) {
    lines.push(`  - ${domain}`);
  }
  lines.push("");
  return lines.join("\n");

/**
 * Backlog tool wrapper generator - local work queue.
 *
 * Generates a bash script that manages a local JSON-based work queue.
 * Internal agent backlog - always available, no external service needed.
 * Tasks are stored in workspace/memory/tasks.json.
 */

export function generateBacklogTool(): string {
  return `#!/usr/bin/env bash
# backlog - local work queue for autonomous task execution
...
case "\${1:-}" in
  ...
  count)   cmd_count ;;
  -h|--help|help|"") usage ;;
  *) echo "backlog: unknown command '$1'" >&2; usage >&2; exit 1 ;;
esac
`;
}

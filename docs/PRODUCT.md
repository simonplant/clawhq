...
OpenClaw already has a control panel (the Gateway UI). That's cPanel. ClawHQ is WordPress. We have hundreds of recipes — complete operational profiles for every use case — and during setup we cook them personalized for you:
...
Six modules. One binary. Parallel development tracks.
...
- [ ] **AI-powered config inference** `should`
...
- [ ] **Config generation with landmine prevention** `must`
  Every generated config passes all 14 landmine rules. Generation cannot produce a broken config. Output: `openclaw.json`, `.env`, `credentials.json`, `docker-compose.yml`, `Dockerfile`, identity files, tools, skills, cron jobs.
  - _Impl note: See OPENCLAW-REFERENCE.md → The 14 Configuration Landmines._

- [ ] **Integration auto-detection** `should`
...
- [ ] **PII and secret scanning** `should`
...
- [ ] **Predictive health alerts** `should`
...
- [ ] **Encrypted backup and restore** `must`

...
> **Implementation status key used in this document:**
>
> - ✅ **Implemented** — Schema field is parsed, validated, and used to generate working output today.
> - 🔜 **Planned** — Field is defined in the schema and accepted by the parser, but the runtime behavior it describes is not yet active. Values you provide are stored and will take effect when the feature ships. See [ROADMAP.md](ROADMAP.md) for timeline.

No key = fully implemented.
...
> **Status:** These fields are parsed and stored but the memory lifecycle they describe — automatic summarization, tier promotion, and retention pruning — is not yet wired to the running agent. Agents currently use a flat `memory/` workspace directory. The three-tier system will activate in a future release without requiring blueprint changes.
...
When ClawHQ forges an agent today, it generates these files in a directory you specify (or the current directory):
...
└── workspace/                 # Agent's world (mounted into container)
    ...
    ├── tools/                 # Generated CLI wrappers (bash/python3)
    │   ├── email
    │   ├── calendar
    │   ├── tasks
    │   └── web-search
...
| File | Mode | Rationale |
|------|------|-----------|
| `.env` | `0600` | Contains API keys and tokens |
| `workspace/SOUL.md` | Read-only mount | Prevents agent self-modification |
| `openclaw.json` | Read-only mount | Prevents agent config tampering |

---

### Planned Directory Structure 🔜

The deployment directory layout below describes the target state documented in [ROADMAP.md](ROADMAP.md) ("Next" milestone: dedicated `~/.clawhq/` deployment directory + separate credential store). It is not yet generated by `clawhq init`. The structure will be adopted in a future release — existing deployments will be migrated automatically.
...
│   ├── docker-compose.yml         # Hardened container config
│   ├── Dockerfile                 # Stage 2 custom layer (tools, binaries)
│   └── credentials.json           # Integration credentials (mode 0600, separate from .env)
│
├── workspace/                     # Agent's world (mounted into container)
...
│   └── memory/                    # Three-tier memory (when memory_policy activates)
│       ├── hot/                   # Recent context
...
│
├── security/
│   └── posture.yaml               # Container hardening level
│
├── ops/

...
> The capability and persona catalog is planned but not yet implemented. The current schema uses `toolbelt.tools[]`, `skill_bundle.included[]`, and `personality.dimensions` directly. See [docs/design/PERSONALITY-MODEL.md](design/PERSONALITY-MODEL.md) for the full design specification.
